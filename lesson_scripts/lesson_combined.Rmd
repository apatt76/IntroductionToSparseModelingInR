...
LASSO, or the Least Absolute Shrinkage and Selection Operator model, is a sparse modeling method that was introduced by Tibshirani (1996). In general, LASSO and other sparse methods are useful when you have many potential explanatory covariates and only a few are important. These methods also excel in cases where there are a high number of covariates compared to the number of data points. Each sparse modeling method has its own way of selecting key parameters from the 'white noise' of multiple unimportant parameters.

LASSO identifies key parameter values by minimizing the residual sum of squares [$\text{RSS} = \sum_{i=1}^n (y_i - \hat{y}_i)^2$] *plus* another shrinkage penalty term, $\lambda \cdot \left| \sum \text{vector of coefficients ($\beta$s)} \right|$. This shrinkage penalty reduces all coefficient estimates towards zero, and some are set to exactly zero. The non-zero coefficients ($\beta$s) remaining are the selected parameter values.  In this process, lambda ($\lambda$) is a tuning parameter that controls the strength of the regularization. In other words, the value of $\lambda$ determines how many parameters will be set to 0: higher values of $\lambda$ result in more shrinkage and more coefficients being set exactly to zero (this is NOT in a 1:1 relationship with the value of $\lambda$). Ideally, select a $\lambda$ that will minimize the shrinkage-penalty equation. The glmnet package has a function, 'cv.glmnet()' which we will use to find the best $\lambda$ value for our regularization and parameter selection. 
